...
def main():
    ...
    env = suite_gym.load(environment_name,
                         gym_env_wrappers=[lambda env: gym.wrappers.AtariPreprocessing(env,
                                                                                       ...
                                                                                       noop_max=0),
                                           lambda env: ReducedActionSpace(
                             env, scale=args.scaling),
                             FrameStack4])
    ...
    if args.model == "quantum":
        ...
        layer5 = keras.layers.Dense(
            args.n_qubits * args.n_layers, activation=args.activation)
        layer6 = ReUploadingPQC(qubits, args.n_layers,
                                observables, seed=args.seed, name="pqc")
        action = keras.layers.Dense(3, activation="linear")
        # action = Rescaling(input_dim=num_actions)

        q_net = sequential.Sequential(
            [preprocessing_layer, layer1, layer2, layer3, layer4, layer5, layer6, action])
        ...
        layer4 = keras.layers.Flatten()
        bottleneck_layer = keras.layers.Dense(
            args.n_qubits * args.n_layers, activation=args.activation)
        ...
        if args.bottleneck:
            q_net = sequential.Sequential(
                [preprocessing_layer, layer1, layer2, layer3, layer4, bottleneck_layer, layer5, action])
            ...
            q_net = sequential.Sequential(
                [preprocessing_layer, layer1, layer2, layer3, layer4, layer5, action])
        ...
        raise ValueError(
            f"Invalid model: {args.model}. Expected 'quantum' or 'classic'.")
    ...
    def train_agent(n_iterations):
        ...
        while iteration < n_iterations:
            ...
            if time_step.is_last().numpy()[0]:
                ...
                processed_observation = tf.convert_to_tensor(
            ...
            if iteration % 10000 == 0:
                ...
                utils.save_outputs(intermediate_models,
                                   time_step.observation, iteration, log_dir)
